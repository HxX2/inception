...
su www-data -s /bin/bash -c "wp core install --path=/var/www/html --url=$WP_URL --title=$DOMAIN_NAME --admin_user=$WP_USER --admin_password=$WP_PASSWORD --admin_email=$WP_EMAIL"
